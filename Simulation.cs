    ...
    public class Simulation
    {
        ...
        public void Start()
        {
            ...
            for (int nShoe = 1; nShoe <= conf.SimulationTotalShoes; nShoe++)
            {
                ...
                {
                    ShoeEditor.Edit(shoe, conf.CounterT, conf.ShoeTrueCounter);
                    f_OneShot = true;
                }
                shoe.Shuffle();
        ...
        private void PlayShoe(Shoe shoe, bool f_OneShot=false)
        {
            ...
            while (cardSequence.Count >= (conf.ShoeDeckTotalNumber - conf.ShoeDeckToExtractNumber) * CardDeck.CARD_NUMBER)
            {
